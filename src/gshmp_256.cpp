...
//     size_t hash = (shkey.token_id * 11400714819323198485ull) >> (64 - 20);

//   
//     for (int i = 0; i < 8; ++i) {
//         hash ^= static_cast<size_t>(shkey.hmac[i]) << ((i % sizeof(size_t)) * 8);
//     }
...
static size_t hash_function(shkey_t shkey) {
    // token_id (8B) + hmac (32B) = 40B
    std::array<uint8_t, 40> input;
    std::memcpy(input.data(), &shkey.token_id, sizeof(shkey.token_id));
    std::memcpy(input.data() + sizeof(shkey.token_id), shkey.hmac, sizeof(shkey.hmac));
...
gshm::gshm() {
    ...
    *next_free_offset = DATA_SEG_START + host_id * DATA_PARTITION_SIZE;
    global_key.token_id = 0; 
    std::memset(global_key.hmac, 0, sizeof(global_key.hmac));
    open_cxl_device();
}
gshm::~gshm() {
...
shkey_t gshm::generate_shkey() {
    ...
    // Step 1: 64-bit token_id
    uint64_t token_id = (static_cast<uint64_t>(rand()) << 32) | rand();
    shkey.token_id = token_id;

    if (use_mac) {
        // Step 2:  token_id and secret key 
        uint8_t input[12];  // 8 bytes token_id + 4 bytes secret
        std::memcpy(input, &token_id, sizeof(token_id));
        std::memcpy(input + sizeof(token_id), &SHKEY_SECRET_KEY, sizeof(SHKEY_SECRET_KEY));

        // Step 3:SHA256 
        SHA256(input, sizeof(input), shkey.hmac);
    } else {
        for (int i = 0; i < 32; ++i) {
            shkey.hmac[i] = static_cast<uint8_t>(rand() % 256);
        }
    }
...
void* gshm::get_data_vaddr_by_offset(size_t offset) {
    return static_cast<char*>(data_addr) + offset;
}
...
void gshm::load_trace_and_insert(const std::string& trace_file, gshm * store) {
    ...
    while (std::getline(infile, line)) {
        ...
        store->insert(key_buf, value_buf);
...
void gshm::clear_protection_table() {
    for (size_t i = 0; i < HASH_TABLE_SIZE; ++i) {
        protection_table[i].valid.store(false);  
    }
